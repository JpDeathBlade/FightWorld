		...
		PseudoInfiniteViewport.Pannable
{
	...
	public GridPanel(DisplayMap map, ResourceBundle res)
	{
		...
		try
		{
			backdrop = ImageIO.read(ClassLoader.getSystemResource(resources
					.getString("backdrop")));
		} catch (IOException e)
		{
	...
	public void paintComponent(Graphics g)
	{
		...
		g2.fillRect(insets.left, insets.top, numCols * (cellSize + 1) + 1,
	...
	private void drawOccupant(Graphics2D g2, int xleft, int ytop, Object obj)
	{
		...
		if (cellToDraw.intersects(g2.getClip().getBounds()))
		{
			...
			{
				Color temp = g2copy.getColor();
				Grunt hp = (Grunt) obj;
				g2.drawImage(hp.getOverlay(), cellToDraw.x, cellToDraw.y,
						cellSize, cellSize, null);
				g2copy.setColor(Color.DARK_GRAY);
				g2copy.fillRect(cellToDraw.x + 1, cellToDraw.y + 1,
						cellSize - 3, 4);
				double hpPercent = hp.getHp() / (double) hp.getMaxHp();
				if (hpPercent > .5)
					g2copy.setColor(Color.GREEN);
				else if (hpPercent > .3)
					g2copy.setColor(Color.YELLOW);
				else
					g2copy.setColor(Color.RED);

				g2copy.fillRect(cellToDraw.x + 1, cellToDraw.y + 1,
						(int) ((hpPercent) * cellSize - 3), 4);
				g2copy.setColor(Color.BLACK);
				g2copy.drawRect(cellToDraw.x + 1, cellToDraw.y + 1,
						cellSize - 3, 4);

				g2copy.drawImage(hp.getStatusManager().getStatusImage(),
						cellToDraw.x, cellToDraw.y + 32, 48, 16, null);
				if (!(hp instanceof Egg))
				{
					g2copy.setColor(Color.BLACK);
					g2copy.drawString("" + hp.getLevel(), cellToDraw.x + 4,
							cellToDraw.y + cellSize - 2);
					g2copy.setColor(Color.WHITE);
					g2copy.drawString("" + hp.getLevel(), cellToDraw.x + 3,
							cellToDraw.y + cellSize - 3);
				}

				g2copy.setColor(temp);
			}
			g2copy.dispose();
		}
	}
	...
	//
	// int miny = Math.max(0, (curClip.y - top) / (cellSize + 1)) * (cellSize +
	// 1) + top;
	// int minx = Math.max(0, (curClip.x - left) / (cellSize + 1)) * (cellSize +
	// 1) + left;
	...
	private void drawWatermark(Graphics2D g2)
	{
		...
		final int WATERMARK_FONT_SIZE = 0;
	...
	private String getToolTipText(Location loc)
	{
		...
		if (f != null)
			return MessageFormat.format(resources
					.getString("cell.tooltip.nonempty"),
					new Object[] { loc, f });
		else
			return MessageFormat.format(resources
	...
	{
		return new Dimension(DEFAULT_CELL_COUNT * (DEFAULT_CELL_SIZE + 1) + 1
				+ extraWidth(), DEFAULT_CELL_COUNT * (DEFAULT_CELL_SIZE + 1)
				+ 1 + extraHeight());
	}
