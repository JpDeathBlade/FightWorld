...
{
	private static final String DEFAULT_MESSAGE = "Let the fighting begin!";
	...
	public void step()
	{
		...
		if (gr instanceof ColorGrid)
		{
			ColorGrid<Actor> cg = (ColorGrid) gr;
			if (cg.getColorCount() > 1)
				winShown = false;
			if (cg.getColorCount() == 1 && !winShown)
			{
				Set<Color> colors = cg.getGridColors();
				Object[] msg = { "The winner is:",
						new SolidColorIcon(colors.iterator().next()) };
